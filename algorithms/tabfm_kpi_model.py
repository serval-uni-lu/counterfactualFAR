...
# tabfm/__init__.py only aliases tabfm_v1_0_0_pytorch as an attribute of the
# package (via `from tabfm.src.pytorch import tabfm_v1_0_0 as tabfm_v1_0_0_pytorch`),
# it isn't a real importable submodule path — import the actual module instead.
from tabfm.src.pytorch.tabfm_v1_0_0 import load as load_tabfm
...
class TabFMKPIModel:
    """
    TabFM (Google Research) regressor integrates KPI generation internally.
    Mirrors TabPFNKPIModel/TabICLKPIModel — same fit/predict interface, same
    internal KPI pipeline.

    Also a pretrained in-context-learning tabular foundation model — same
    memory-bounding knobs (sample_pct at fit(), batched predict()) as the other
    two. Unlike TabPFN/TabICL, tabfm.load()'s own use_cache=True keeps a single
    loaded set of pretrained weights per (model_type, device, dtype) for the
    whole process, so rebuilding TabFMKPIModel every window (as recommendation.py
    does per date) does not reload weights onto the GPU each time.
    """
    ...
    def __init__(
        ...
        if device is None:
            if not torch.cuda.is_available():
                raise RuntimeError(
                    "TabFMKPIModel defaults to GPU (device='cuda') but no CUDA device "
        ...
        self.model = self.pipeline.named_steps["tabfm"]
    ...
    def _select_features_in_order(self, df):
        ...
        if missing:
            raise ValueError(f"Missing KPI features for TabFM model: {missing}")
    ...
    def fit(self, time_series_df, y, kpi_features, artifact_label=None):
        ...
        if isinstance(y, pd.DataFrame):
            ...
            if missing_target_cols:
                raise ValueError(
                    f"Missing required target columns for TabFM training: {missing_target_cols}"
                )
            ...
            if self.sample_pct is not None:
                # Bounds the actual training context TabFM conditions on at
                # predict() time (cost scales with context size), not just a
                # diagnostic — real recommendations train on this sampled set
                # too. Stratified per asset so none drop out.
    ...
    def predict(self, time_series_df):
        ...
        # downstream — filtered unconditionally, same as TabPFNKPIModel.
    ...
    def _predict_in_batches(self, X):
        ...
        # real score, just computed in GPU-memory-bounded chunks instead of
