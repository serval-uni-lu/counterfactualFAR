...
class TabICLKPIModel:
    """
    TabICL regressor integrates KPI generation internally.
    Mirrors TabPFNKPIModel/RFRKPIModel/LGBMKPIModel — same fit/predict interface,
    same internal KPI pipeline.

    Like TabPFN, TabICL is a pretrained in-context-learning tabular foundation
    model, not trained from scratch: fit() just caches the training set, predict()
    does the real work in a forward pass conditioned on it — cost scales with
    context (training set) size, so large windows can exhaust GPU memory. Same two
    memory-bounding knobs as TabPFNKPIModel:
      - sample_pct: fit() trains on a per-asset stratified subsample of the window
        instead of the full thing.
      - predict_batch_size: predict() only scores the most recent date's rows and
        does so in GPU-bounded batches rather than one giant forward pass.
    """
    ...
    def __init__(
        ...
        if device is None:
            if not torch.cuda.is_available():
                raise RuntimeError(
                    "TabICLKPIModel defaults to GPU (device='cuda') but no CUDA device "
        ...
        self.model = self.pipeline.named_steps["tabicl"]
    ...
    def _select_features_in_order(self, df):
        ...
        if missing:
            raise ValueError(f"Missing KPI features for TabICL model: {missing}")
    ...
    def fit(self, time_series_df, y, kpi_features, artifact_label=None):
        ...
        if isinstance(y, pd.DataFrame):
            ...
            if missing_target_cols:
                raise ValueError(
                    f"Missing required target columns for TabICL training: {missing_target_cols}"
                )
            ...
            if self.sample_pct is not None:
                # Bounds the actual training context TabICL conditions on at
