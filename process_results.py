...
def save_all_models_plots(all_run_stats_tables):
    ...
    for metric_name in metrics:
        ...
        n_models = len(metric_df)
        fig, ax = plt.subplots(figsize=(10, max(4, n_models * 0.5)))
        ax.barh(metric_df["run"], metric_df["mean"])
        ax.set_title(f"all_models | {metric_name} | mean", fontsize=14)
